...
const meta = {
  title: "Widgets/dummy-counter",
  argTypes: { ...storyArgTypes(dummyCounter), initial: { control: "number", description: "Seeded store value" } },
  args: { ...storyArgs(dummyCounter), initial: 0 } as Args,
  render: ({ initial, ...settings }) => (
    // Its own reaction stores the emitted value, exactly as on a page.
    <WidgetStory
      key={String(initial)}
      definition={dummyCounter}
...
export const Clicks: Story = {
  play: async ({ canvas, userEvent }) => {
    const button = canvas.getByTestId("dummy-counter");
