/** Crash dummy — throws during render to exercise the engine's per-cell
...
function DummyCrash({ viewModel }: WidgetProps<VM, typeof NO_EVENTS>): never {
  throw new Error(viewModel.message);
}

export const dummyCrash = defineWidget({
  type: "dummy-crash",
  description: "Throws on render — proves the per-cell error boundary (testing only)",
  io: NO_IO,
  events: NO_EVENTS,
  viewModel,
  component: DummyCrash,
});
