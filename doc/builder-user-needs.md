...
| ID | Want · *done when* | Priority | Today | What it takes | Size | Depends on |
|---|---|---|---|---|---|---|
| W12 | Connect two widgets by picking the other one. *Done when* a counter and a display are wired with zero typing. | Must | Partly — a path another widget WILL write is not offered until someone has used it. Built: every input port starts from a GENERATED path, `<page>.<widget name>.<port>` (`builder.refresher.schedule`, the next one `builder.refresher2.…`; `suggestedInputPaths` in the engine), which the user keeps, changes or replaces with existing data. Not built: the reaction that writes a controlled widget's value still needs its path typed | a "wiring index" derived from the page plan: who reads and who writes each path; a picker "value of ‹widget title›"; a controlled widget's port-and-event pair becomes ONE field with a generated path. **New, derived only** | M | Names |
